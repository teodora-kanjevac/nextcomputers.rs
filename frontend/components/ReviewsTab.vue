        ...
        <section class="p-8">
            <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
                <div class="flex items-center gap-3">
                    ...
                    <StarRating :rating="averageRating" :total-reviews="totalReviews" class="pb-1" />
                </div>

                <div class="my-6 gap-8 sm:flex sm:items-start md:my-8">
                    <div class="shrink-0 space-y-4">
                        <p class="ms-0.5 text-2xl font-semibold leading-none text-gray-900">{{ averageRating.toFixed(2)
                            }} od 5
                        </p>
                        <button type="button" data-modal-target="review-modal" data-modal-toggle="review-modal"
                            class="mb-2 me-2 rounded-lg bg-primary-light px-5 py-2.5 text-sm font-medium text-white hover:bg-rose-800">
                            Napiši recenziju
                        </button>
                    </div>

                    <div class="mt-6 min-w-0 flex-1 space-y-3 sm:mt-0">
                        <div v-for="(rating, index) in ratings" :key="index" class="flex items-center gap-1.5">
                            <p class="w-2 shrink-0 text-start text-sm font-medium leading-none text-gray-900">{{
                                rating.stars }}</p>
                            <StarFilledIcon class="size-5" />
                            <div class="h-1.5 w-80 rounded-full bg-gray-200">
                                <div class="h-1.5 rounded-full bg-yellow-300"
                                    :style="{ width: calculatePercentage(rating.count) + '%' }"></div>
                            </div>
                            <a href="#"
                                class="w-8 shrink-0 ps-1 text-right text-sm font-medium leading-none hover:text-primary sm:w-auto sm:text-left">
                                {{ rating.count }} <span class="hidden sm:inline">recenzije</span>
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-20 divide-y divide-gray-200">
                    <div v-for="(userReviews, index) in visibleUserReviews" :key="index"
                        class="gap-3 py-6 sm:flex sm:items-start">
                        <div class="shrink-0 space-y-2 sm:w-48 md:w-72">
                            <div class="flex items-center">
                                <UserStarRating :rating="userReviews.rating" class="-ms-0.5" />
                            </div>

                            <div class="space-y-0.5">
                                <p class="text-base font-semibold text-gray-900">{{ userReviews.name }}</p>
                                <p class="text-sm font-normal text-gray-500">{{ userReviews.date }}</p>
                            </div>
                        </div>

                        <div class="mt-5 min-w-0 space-y-4">
                            <p class="text-base font-normal text-gray-700">{{ userReviews.comment }}</p>
                        </div>
                    </div>

                </div>
                <div class="mt-6 text-center" v-if="visibleReviews < userReviews.length">
                    <button type="button" @click="loadMoreReviews"
...
const averageRating = ratings.reduce((total, rating) => total + (rating.stars * rating.count), 0) / totalReviews;

const calculatePercentage = (ratingCount) => (ratingCount / totalReviews) * 100;

const visibleReviews = ref(4);

const visibleUserReviews = computed(() => {
    return userReviews.value.slice(0, visibleReviews.value);
});
