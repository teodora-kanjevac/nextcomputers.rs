<template>
    <div>
        <h2 :id="drawerHeadingId">
            <button
                type="button"
                class="flex items-center justify-between w-full py-1.5 px-2 mt-1 mb-2 font-semibold rounded hover:bg-primary-light hover:text-gray-100"
        ...
        <div :id="drawerBodyId" class="hidden" :aria-labelledby="drawerHeadingId">
            <template v-for="subcategory in category.subcategories" :key="subcategory.id">
                <NuxtLink to="/" @click="closeDrawer" class="p-2 ps-6 rounded block border-b border-gray-200 font-semibold text-sm text-gray-800 hover:bg-gray-200">
                    {{ subcategory.name }}
                </NuxtLink>
            </template>
        </div>
