        ...
        <div :id="drawerBodyId" class="hidden" :aria-labelledby="drawerHeadingId">
            <template v-for="subcategory in category.subcategories" :key="subcategory.id">
                <a
                    ...
                    @click="closeDrawer"
                    class="p-2 ps-6 rounded block border-b border-gray-200 font-medium text-sm text-gray-800 hover:bg-gray-200">
                    {{ subcategory.name }}
                </a>
            </template>
        </div>
...
const drawerHeadingId = computed(() => `drawer-heading-${sanitizeId(category.name)}`)
const drawerBodyId = computed(() => `drawer-body-${sanitizeId(category.name)}`)
